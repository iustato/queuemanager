...
// expKey: 8 bytes big-endian expiresAtMs + msg_id
...
// key: 8 bytes big-endian failedAtMs + msg_id
// enqFailKey: 8 bytes big-endian enqueueFailedAtMs + msg_id bytes
func makeEnqueueFailedKey(enqueueFailedAtMs int64, msgID string) []byte {
	b := make([]byte, 8+len(msgID))
...
func parseEnqueueFailedKey(k []byte) (enqueueFailedAtMs int64, msgID string, ok bool) {
	if len(k) < 8 {
		return 0, "", false
	}
